...
cartRouter.post("/", validateCartPost, async (request: Request, response: Response) => {
  ...
  if (!requestedProductsExist(requestCart.products, allProducts)) {
    response.status(400).send({message: "Some products in the cart were not found"});
    return;
  }
  ...
  if (!formattedCart.products.every((product) => product.realStock >= product.quantity)) {
    response.status(400).send({message: "Some products in the cart do not have enough stock"});
    return;
  }
  ...
  if (cheapestTariff.price) {
    ...
    response.status(400).send({message: "No tariffs are available"});
  }
  return;
